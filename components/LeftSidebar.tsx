        ...
        <section className={cn('left_sidebar h-[calc(100vh-5px]', {'h-[calc(100vh-140px)]':audio?.audioUrl})}>
            <nav className="flex flex-col gap-6">
                <Link href="/" className="flex cursor-pointer items-center gap-1 pb-10 max-lg:justify-center">
                    ...
                    <h1 className="text-24 font-extrabold text-white max-lg:hidden">EcoPodcast</h1>
                </Link>
                {sidebarLinks.map(({ route, label, imgURL }) => {
                    ...
                    return <Link href={route} key={label} className={cn("flex gap-3 items-center py-4 max-lg:px-4 justify-center lg:justify-start", {
                        ...
                        <Image src={imgURL} alt={label} width={24} height={27} />
                        <p>{label}</p>
                    </Link>
                })}
            </nav>
            ...
            <SignedIn>
                <div className="flex-center w-full pb-14 max-lg:px-4 lg:pr-8">
                    <Button className="text-16 w-full bg-orange-1 font-extrabold" 
                        onClick={ ()=> signOut( ()=> router.push("/") ) }>Log Out
                    </Button>
                </div>
            </SignedIn>
        </section>
